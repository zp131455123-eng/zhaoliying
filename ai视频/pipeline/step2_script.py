...
def call_claude(hotspots_text: str) -> str:
    """调用Claude生成文案"""
    prompt = PROMPT_TEMPLATE.format(hotspots=hotspots_text)
...
def main(hotspots_file=None):
    ...
    items = data["items"][:20]  # 最多传20条给LLM选题
    hotspots_text = "\n".join(
        f"{i+1}. [{item['source']}] {item['title']}" + (f" - {item['desc']}" if item.get('desc') else "")
        for i, item in enumerate(items)
    )

    print(f"  输入热点数: {len(items)} 条")

    # 调用Claude
    raw = call_claude(hotspots_text)
    print("  ✅ Claude返回成功")
